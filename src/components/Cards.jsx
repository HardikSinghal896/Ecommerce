...
import passport1 from '../Images/-6127404330106600000_120.jpg'
...
const Cards = ({ cardArray }) => {
  ...
  const [arr, setArr] = useState(cardArray);
  ...
  useEffect(() => {
    ...
    if (paramT === null) {
      ...
      setArr(cardArray.filter((item) => {
        return item.type == "rooms"
      }))
    } else if (paramT == "n") {
      setArr(cardArray.filter((item) => {
        return item.type == "room_mates"
      }))
    } else if (paramT == "p") {
      setArr(cardArray.filter((item) => {
        return item.type == "pg"
    ...
    <div className={style.mainContainer}>
      {
        arr.map((item) => {
          return (
            <Card title={item.title} location={item.location} />
          )
        })
      }
    </div>
...
const Card = ({ title, location }) => {
  ...
  function handleCardClick() {
    ...
    console.log(item, "item in cards")
    if (item == null) {
      setIsOpen(true);
    }
    else {
  ...
  return (
    <>
      {isOpen && <OpenModel setIsOpen={setIsOpen} />}
      <div className={style.container} onClick={handleCardClick}>
        <div>
          <img src={passport1} alt="" height={"100px"} />
        </div>
        <div>
          <h3>Hardik Singhal</h3>
          <p>{location}</p>
